...
namespace EnhancedFramework.Core {
    ...
    public static class Mathm {
        ...
        /// </summary>
        /// <param name="_collection">Collection to get a random element from.</param>
        /// <param name="_getProba">Getter of a single element probability (element as parameter).
        ...
        public static int RandomProbability<T>(IList<T> _collection, Func<T, float> _getProba, bool _ignoreZeroProba = false) {
            ...
            for (int i = 0; i < _collection.Count; i++) {
                float _probability = _getProba(_collection[i]);
                _sum += _probability;

                _buffer.Add(_probability);
            }

            // 3. Generate a random value using the sum of all probabilities.
            // 4. Iterate over all elements, and select the first one with a probability below the generated number.
