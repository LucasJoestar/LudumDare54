...
namespace EnhancedFramework.Core.GameStates {
    ...
    public class GameStateManager : EnhancedSingleton<GameStateManager>, IStableUpdate {
        ...
        void IStableUpdate.Update() {
            ...
            // Push.
            if (pushPendingStates.Count != 0) {

                for (int i = 0; i < pushPendingStates.Count; i++) {
                    PushState(pushPendingStates[i], false, false);
                }

                pushPendingStates.Clear();
                _refresh = true;
            }

            // Pop.
            if (popPendingStates.Count != 0) {

                for (int i = 0; i < popPendingStates.Count; i++) {
                    PopState(popPendingStates[i], false, false);
                }

                popPendingStates.Clear();
                _refresh = true;
            }
        ...
        private void RefreshCurrentState() {
            ...
            for (int i = 0; i < overrideCallbacks.Count; i++) {
                IGameStateOverrideCallback _callback = overrideCallbacks[i];
