...
namespace EnhancedEditor.Editor {
    ...
    public class SceneDesignerEnhancedSettings : EnhancedSettings {
        ...
        internal static void DrawSceneEditorSettings()
        {
            ...
            void DrawField(GUIContent _label, SerializedProperty _toggleProperty, SerializedProperty _rangeProperty, float maxValue)
            {
	...
	public class SceneDesigner : EditorWindow {
        ...
        private class MeshInfo {
            ...
            public MeshInfo(SpriteRenderer sprite)
            {
                Sprite = sprite;
                Transform = sprite.transform;
                IsSprite = true;

                Materials = sprite.sharedMaterials;
        ...
        private static List<GameObject> historical = new List<GameObject>();
        ...
        private static void OnSceneGUI(SceneView _sceneView) {
            ...
            foreach (MeshInfo _mesh in meshInfos) {
                ...
                for (int _i = 0; _i < _mesh.Materials.Length; _i++) {
                    ...
                    if (_mesh.IsSprite)
                    {
                        Draw(_mesh.Sprite, _matrix, _material, 2, _camera, _i);
                    }
                    else
                    {
        ...
        public static void Draw(SpriteRenderer sprite, Matrix4x4 matrix, Material material, int layer, Camera camera, int subMeshIndex)
        {
            if (quadMesh == null)
            {
                quadMesh = CreateQuad();
            ...
            mpb.SetColor("_Color", sprite.color);

            Vector3 scale    = Vector3.Scale(matrix.lossyScale, sprite.localBounds.size);
            Vector3 position = matrix.GetPosition() + (matrix.rotation * sprite.localBounds.center);
            ...
            matrix = Matrix4x4.TRS(position, matrix.rotation, scale);
            Graphics.DrawMesh(quadMesh, matrix, material, layer, camera, subMeshIndex, mpb);
        }
        ...
        public class SceneDesignerQuickAccess : EditorWindow
        {
            #region Window GUI
